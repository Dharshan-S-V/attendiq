...
export default function Attend() {
  ...
  function allowGPS() {
    ...
    // ── Robust multi-sample GPS with outlier filtering ──
    // Take up to 8 readings over 5s, discard bad ones (accuracy > 35m),
    // then use MEDIAN lat/lng (resistant to wild outliers).
    const samples = [];
    const MAX_SAMPLES = 8;
    const COLLECT_MS = 5000;  // 5 seconds of collection
    const MAX_ACCURACY = 35;    // discard readings worse than 35m
    let finalized = false;
    ...
    const watchId = navigator.geolocation.watchPosition(
      pos => {
        ...
        const good = samples.filter(s => s.accuracy <= MAX_ACCURACY).length;
        setLocMsg(`Locking GPS… ${good} good reading${good !== 1 ? 's' : ''} (~${Math.round(accuracy)}m accuracy)`);
      ...
      err => {
        navigator.geolocation.clearWatch(watchId);
        clearTimeout(timer);
        if (samples.length > 0) finalizeSamples();
        else {
          setLocState('err');
    ...
    const timer = setTimeout(() => {
      ...
      if (samples.length > 0) finalizeSamples();
        ...
        setLocState('err');
        setLocMsg('Could not get GPS fix. Move near a window and try again.');
        setSubmitting(false);
      }
    }, COLLECT_MS);
    ...
    function finalizeSamples() {
      if (finalized) return; // prevent double-fire
      finalized = true;

      // Filter to only good-accuracy readings; fall back to all if none pass
      let usable = samples.filter(s => s.accuracy <= MAX_ACCURACY);
      if (usable.length === 0) usable = samples;

      // Use MEDIAN coordinates — robust against outlier readings
      const lat = median(usable.map(s => s.latitude));
      const lng = median(usable.map(s => s.longitude));
      const bestAccuracy = Math.min(...usable.map(s => s.accuracy));

      setLocState('ok');
      setLocMsg(`Location locked (${usable.length} readings, ~${Math.round(bestAccuracy)}m). Submitting…`);
      doSubmit(lat, lng, bestAccuracy);
    }
  }
        ...
        <div ref={pageRef} style={{ ...S.page, overflowY: 'auto' }}>
          ...
          <div style={{ padding: '0 16px 60px', maxWidth: '500px', margin: '0 auto' }}>
            <div style={{ ...S.resultCard, animation: 'fadeUp .5s ease' }}>
              <div style={{ fontSize: '72px', marginBottom: '14px', animation: 'pop .6s cubic-bezier(.34,1.56,.64,1)', filter: 'drop-shadow(0 0 15px rgba(255,255,255,0.4))' }}>{ok ? '✨' : '📍'}</div>
              ...
              <p style={{ color: 'var(--txt3)', fontSize: '14px', marginBottom: '24px', lineHeight: '1.6' }}>
                {ok
                  ? 'Your attendance has been successfully recorded. You may close this page.'
                  : 'You appear to be outside the allowed zone. Please move closer to the classroom and try again.'
                }
              </p>
              ...
              <div style={S.detailCard}>
                {[
                  ...
                  <div key={k} style={S.detailRow}>
                    ...
                    {k === 'Status'
                      ? <span style={{ fontSize: '13px', fontWeight: 600, color: ok ? '#248a3d' : '#f5a623' }}>{ok ? '● Present ✨' : '● Out of Range 📍'}</span>
