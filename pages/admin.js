...
export default function Admin() {
  ...
  function myLocation() {
    ...
    const watchId = navigator.geolocation.watchPosition(
      p => {
        samples.push({ lat: p.coords.latitude, lng: p.coords.longitude, acc: p.coords.accuracy });
        const good = samples.filter(s => s.acc <= MAX_ACCURACY).length;
        showToast(`Locking GPS… ${good} good reading${good !== 1 ? 's' : ''}`);
      ...
      e => {
        navigator.geolocation.clearWatch(watchId);
        clearTimeout(timer);
        if (samples.length > 0) done();
        else showToast('GPS error: ' + e.message, 'er');
      },
    ...
    const timer = setTimeout(() => {
      navigator.geolocation.clearWatch(watchId);
      if (samples.length > 0) done();
      else showToast('GPS timeout — try near a window', 'er');
    }, COLLECT_MS);
    ...
    function done() {
      if (finalized) return;
      finalized = true;
      let usable = samples.filter(s => s.acc <= MAX_ACCURACY);
      if (usable.length === 0) usable = samples;
      const lat = median(usable.map(s => s.lat));
      const lng = median(usable.map(s => s.lng));
      const bestAcc = Math.min(...usable.map(s => s.acc));
      setForm(f => ({ ...f, lat: lat.toFixed(6), lng: lng.toFixed(6) }));
      showToast(`Location captured (${usable.length} readings, ~${Math.round(bestAcc)}m accuracy)`);
    }
  }
